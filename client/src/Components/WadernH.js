...
function WardenH() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const wardenStudents = response.data.filter(student => student.isTutor === true && student.isHod === true);
        setStudentData(wardenStudents);
  ...
  const approveRequest = async (id) => {
    try {
      const res = await axios.put(`http://localhost:5000/WardenUpdate/${id}`, { isWarden: true });
          ...
          <Table responsive="sm">
            <thead >
              <tr className='bg-warning'>
                ...
                <th style={{ backgroundColor: '#ff8830' }}>ROOM NO</th>
